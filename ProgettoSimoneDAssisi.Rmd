...
ggplot(phishing_data, 
       aes(x = URLSimilarityIndex)) + 
           ...
           labs(title = "Skewness di URLSimilarityIndex", 
        x = "URLSimilarityIndex",
        y = "Densità") + theme_minimal() + annotate("text", x = max(phishing_data$URLSimilarityIndex) * 0.8, y = 0.02, 
           label = paste("Skewness =", round(skewness_url, 4)), color = "red")

kurtosis_url <- kurtosis(phishing_data$URLSimilarityIndex)
cat("Curtosi per URLSimilarityIndex:", round(kurtosis_url, 4), "\n")

# Grafico per la curtosi
ggplot(phishing_data, aes(x = URLSimilarityIndex)) + 
  geom_histogram(
    aes(y = ..density..),
    bins = 10, 
    fill = "#FF5733", 
  ...
  labs(title = "Curtosi di URLSimilarityIndex", 
       x = "URLSimilarityIndex", 
       y = "Densità") + 
  theme_minimal() + 
  annotate("text", x = max(phishing_data$URLSimilarityIndex) * 0.8, y = 0.02, 
           label = paste("Curtosi =", round(kurtosis_url, 4)), color = "blue")
